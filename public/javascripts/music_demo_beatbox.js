T("audio").load("/timbre.js/misc/audio/drumkit.wav", function() {
  var BD  = this.slice(   0,  800).set({bang:false});
  var SD  = this.slice( 800, 1000).set({bang:false});
  var HH1 = this.slice(1000, 1500).set({bang:true, mul:0.4});
  ...
  var arp  = T("OscGen", {wave:"sin(17)", env:env, mul:0.5});

  //
  var table = [200, [4800, 150], [2400, 500]];
  var cutoff = T("sin", {freq:"400ms", mul:800, add:1760}).kr();
  var VCO = T("saw", {gain: 1, mul:0.2});

  T("delay", {time:"BPM128 L4", fb:0.65, mix:0.35}, 
    T("pan", {pos:0.6}, vcf), 
    T("pan", {pos:T("tri", {freq:"BPM64 L8", mul:0.8}).kr()}, arp),
    T("pan", {pos:T("lpf", {cutoff:cutoff, Q:10}, VCO).kr()}, arp)
  ).play();
